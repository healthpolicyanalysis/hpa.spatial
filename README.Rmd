...
hpa.spatial downloads data over HTTPS (from GitHub and from routing servers
via `osrm_table_pairs()`). On some Windows setups, curl's default SSL
backend fails to validate these certificates, raising an SSL-related error.
If you hit this, switch curl to the OpenSSL backend:

``` r
Sys.setenv(CURL_SSL_BACKEND = "openssl")
```

To make this persist across sessions, add `CURL_SSL_BACKEND=openssl` to your
`.Renviron` (see `usethis::edit_r_environ()`).
