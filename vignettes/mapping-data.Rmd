...
sa2_2011_data <- get_polygon("sa22011") |> 
  ...
  mutate(
    ...
    ages_12_to_24 = rnorm(max(n()), mean = 20, sd = 4),
    ages_25_to_64 = rnorm(max(n()), mean = 25, sd = 5),
    ages_65_plus = rnorm(max(n()), mean = 40, sd = 8)
  ) |> 
  pivot_longer(
    !sa2_code_2011, 
    names_to = "agegrp",
    values_to = "outcome_var",
    names_transform = \(x) str_remove(x, "ages_") |> str_replace("_to_", "-") |> str_replace("_plus", "+")
  )
